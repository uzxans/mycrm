<?php
// Например, только активные объекты
$stmt = pdo()->prepare("SELECT COUNT(*) AS count FROM `object` WHERE `status_obj` = 0");
$stmt->execute();
...
$labels = [];
$valuesWithStatus100 = [];
$valuesWithoutStatus100 = [];

foreach ($data4 as $row4) {
    $valuesWithoutStatus100[] = $row4['count'];
}
...
<div class="row">
    ...
    <div class="col-xl-4 col-md-4 col-12 mb-2">
        <form method="POST" class="mt-2" action="upload_photo.php" enctype="multipart/form-data">
            <div class="user_lk_info">
                <div class="img_box_user_info">
                    <div class="box_img">
                        ...
                        if (!empty($user['dir_img'])) {
                            ...
                            $imageSource = "./accets/img/zaglushkaImgBlack.png";
                        }
                        ?>
                        <img id="imgFoto" src="<?php echo $imageSource; ?>" alt="user">
                        <button class="btn btnAddImg" data-bs-toggle="modal" data-bs-target="#staticBackdropImg"><i
                                    class="photo-icon"></i></button>
                    </div>
                    <div class="status_user">
                        ...
                        <p class="status_user">
                            ...
                            if ($user['position'] == 2) {
                                ...
                                echo 'Менеджер отдела продаж';
                            } ?>
                            <button class="btn setting_mob"><i class="time-green-icon"></i></>
                        </p>
                    </div>
                </div>
                <!-- Modal AddImg-->
                <!-- Modal AddImg-->
                <div class="modal fade" id="staticBackdropImg" tabindex="-1" aria-labelledby="exampleModalLabel"
                     aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="text-center mt-3">
                                    <div
                                            style="width:150px; height:150px; border-radius:50%; overflow:hidden; margin:auto; border:2px solid #ddd;">
                                        <img id="previewCircle" style="width:100%; height:100%; object-fit:cover;">
                                    </div>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" id="cropBtn" class="btn btn-primary">Сохранить</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form_lk">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="fio" value="<?= htmlspecialchars($user['name']) ?>">
                    </div>
                    <div class="mb-3">
                        <label for="exampleFormControlInput1" class="form-label">Логин</label>
                        <input type="email" class="form-control" id="exampleFormControlInput1" value="<?= htmlspecialchars($user['username']) ?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Пароль</label>
                        <div class="input-group mb-3">
                            <input class="form-control password" id="password" class="block mt-1 w-full" type="password"
                                   name="password" value="secret!" required />
                            <span class="input-group-text togglePassword" id="">
                          <i class="icon-eye-white" data-feather="eye" style="cursor: pointer"></i>
                        </span>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="exampleFormControlInput1" class="form-label">Номер телефона</label>
                        <input type="tel" class="form-control" id="exampleFormControlInput1" value="<?php  $formattedNumber = formatPhoneNumber($user['phone']);  echo $formattedNumber ?>">
                    </div>
    ...
    <div class="col-xl-8 col-md-8 col-12 mb-2">
        <div class="content_info_user_lk">
            <div class="row">
                ...
                <div class="rox_box col-lg-6 col-xxl-4">
                    <div class="midl_box" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight"
            ...
            <div class="chart_box">
                ...
                <script>
                    ...
                    gradient2023.addColorStop(0, 'rgba(204, 255, 153, 0.6)');
                    gradient2023.addColorStop(1, 'rgba(204, 255, 153, 0)');

                    const gradient2024 = ctx.createLinearGradient(0, 0, 0, 400);
                    gradient2024.addColorStop(0, 'rgba(102, 204, 102, 0.6)');
                    gradient2024.addColorStop(1, 'rgba(102, 204, 102, 0)');

                    const gradient2025 = ctx.createLinearGradient(0, 0, 0, 400);
                    gradient2025.addColorStop(0, 'rgba(0, 102, 102, 0.6)');
                    gradient2025.addColorStop(1, 'rgba(0, 102, 102, 0)');
...
<!--End-ROW-->
    ...
    <!--Modal HR INFO-->
    <div class="offcanvas offcanvas-end hr-info" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title">Мой диск</h5>
        ...
        <div class="offcanvas-body">
            ...
            <div class="search_box">
                <div class="d-flex align-items-center gap-2 mb-3">
                    <div class="dropdown">
                        <button class="btn btn-primary dropdown-toggle btn-send" data-bs-toggle="dropdown">Добавить</button>
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="#" id="createFolderBtn">Создать папку</a></li>
                            <li><label class="dropdown-item" for="uploadFile">Загрузить файл</label></li>
                            <li><a class="dropdown-item" href="#">Загрузить архив zip</a></li>
                        </ul>
                    </div>
                    <input type="file" id="uploadFile" hidden>
                    <input type="text" id="searchBox" class="form-control search" placeholder="Поиск">
                    <button class="btn btn-outline-danger btn-white" id="trashBtn">Корзина</button>
                </div>
            </div>

            <!-- Список файлов -->
            <div id="fileList" class="list-group">
                <!-- Файлы/папки будут добавляться AJAX’ом -->
            </div>

            <!-- Кнопки действий -->
            <div class="mt-3 d-flex gap-2">
                <button class="btn btn-outline-secondary" id="shareBtn" disabled>Поделиться</button>
                <button class="btn btn-outline-danger" id="deleteBtn" disabled>Удалить</button>
            </div>
        </div>
    </div>
    <!--END-Modal HR INFO-->
    ...
    <div class="modal fade" id="createFolderModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input type="text" id="folderNameInput" class="form-control" placeholder="Введите название папки">
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Отмена</button>
                    <button type="button" id="saveFolderBtn" class="btn btn-primary">Создать</button>
                </div>
            </div>
        </div>
    </div>