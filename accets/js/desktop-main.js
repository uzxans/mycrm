...
function handleSelection(value, tab, text) {
    ...
    // вычисляем нормализованную форму и отображаемый текст
    const raw = (value !== undefined && value !== null) ? value : (text || "");
    const norm = normalize(raw);
    const displayText = text || value || raw;

    if (norm === "все" || norm === "выбрать" || norm === "") {
        activeFilters[tab] = [];
        clearTags(tab);
    } else if (!activeFilters[tab].some(v => normalize(v) === norm)) {
        // сохраняем оригинальное значение (можно сохранить нормизированное или оригинал — выбрал хранить оригинал)
        activeFilters[tab].push(raw);
        addTag(raw, tab, displayText);
    }
...
document.querySelectorAll(".option").forEach(option => {
    option.addEventListener("click", () => {
        ...
        const tab = option.closest(".tab-content").id;
        const text = option.textContent.trim();
        handleSelection(value, tab, text);
    });
});
...
document.querySelectorAll(".tab-content select").forEach(select => {
    select.addEventListener("change", () => {
        ...
        const value = selected.dataset.value ?? selected.value ?? selected.text;
        const tab = select.closest(".tab-content").id;
        const text = selected.textContent?.trim() || value;
...
function addTag(value, tab, text) {
    ...
    if ([...tagsContainer.children].some(tag =>
        normalize(tag.dataset.value) === normValue &&
        tag.dataset.tab === tab &&
        normalize(tag.dataset.text || "") === normText
    )) {
        return;
    }
    ...
    tag.querySelector(".remove").addEventListener("click", () => {
        ...
        if (tab === "date") {
            ...
            // удаляем по нормализованному значению
            activeFilters[tab] = (activeFilters[tab] || []).filter(v => normalize(v) !== normValue);
        }
...
function renderBoard() {
  ...
  document.querySelectorAll(".card").forEach(card => {
    ...
    const phone = card.querySelector("[data-phone]")?.dataset.phone || "";
      const objectRaw = card.querySelector("[data-object]")?.dataset.object || "";
      const object = normalize(objectRaw);
    const hr = card.querySelector("[data-hrs]")?.dataset.hr || "";
    ...
    if (activeFilters.phone.length && !activeFilters.phone.some(p => phone.includes(p))) {
      ...
      if (activeFilters.object.length) {
          // нормализуем активные фильтры и смотрим пересечение
          const hasMatch = activeFilters.object.some(f => normalize(f) === object);
...
// Стартовый рендер
console.log("ACTIVE FILTERS (normalized):", JSON.stringify(activeFilters.map ? activeFilters : activeFilters)); // для визуальной проверки
